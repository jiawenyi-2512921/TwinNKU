...
- 校园地图编辑器支持浏览、移动整个点位（锚点与点击范围联动）、单独移动新增文字、两角框选矩形、点击绘制多边形、拖动或删除边界顶点、撤销最近40次已应用的位置调整。多边形绘制中也可以逐点撤销或取消。
...
2026-09-23反馈“审核通过后客户端没有变”后，增加公开地图自动刷新、整体移动点位和发布后公开接口核验。原来的“点击定位”只改变锚点，旧版已打开客户端也不会自动读取发布更新；详细原因、部署与验收见[16-publication-sync-fix.md](16-publication-sync-fix.md)。原图内烧录名称仍不随点位移动，不重新编码图片。
