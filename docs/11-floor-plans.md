> 2026-09-23最新要求：只上传、展示有标注楼层图。新生成器只需要labeled_file；导入支持单张labeled图；旧双图兼容导入但公共API不提供clean。图书馆1层仅标注版revision 2与部署见[当前交付](13-map-interaction-update.md)。下面的强制双图规则为历史设计，已被本条替代。

# M05a 楼层双图查看 · v0.3.0

## 1. 本轮要求与真实状态

按用户最终要求提前实现独立楼层查看，不等待智能体、全景或路线模块。每层由已标注图和无标注图组成，默认展示已标注图；本批运行资源只包含这两种整理图，不含实拍照片。第20项综合实验楼C区未完成，本批排除。

用户现已授权GitHub为图片等素材的交付渠道。本批校园地图v2与图书馆1层双图已上传本仓库Releases，服务器直接下载与导入命令见[GitHub资源交接](12-github-resource-handoff.md)。
