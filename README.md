...
本次是 **M00 基础版本**：响应式网页、FastAPI 服务、数据库模型及迁移、公开点位只读接口、健康检查、OpenAPI/TypeScript 契约、部署文件、CI。

**未实现**：正式地图、真实 AI 对话、真实路线、VR 深度控制、楼层导航、语音、后台审核。生产模式默认关闭这些能力，页面不会使用假回复冒充智能体。数据库初始只有校园元数据；点位需经过后续内容审核录入。
