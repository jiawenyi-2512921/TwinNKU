...
**当前交付 v0.4.0：**在原图文字地图、83个命名地点和有标注楼层查看基础上，增加 `/admin` 管理后台：四种角色、校区/指定点位授权、地图选点与范围编辑、草稿审核发布、下架恢复、账号管理和操作记录。周恩来雕像与原图清晰度保持。AI/VR/主题导览未实现。

**给部署方：**使用 `feat/admin-console` 分支，按[后台部署与账号初始化](docs/15-admin-console.md)升级。后台默认关闭，需在服务器交互创建首个管理员并启用；没有默认密码。已有地图v3与标注楼层v2资源无需重新导入，原始资源仍见[发布页](https://github.com/jiawenyi-2512921/TwinNKU/releases/tag/map-interactions-2026.09.23-v3)。
...
**未实现**：真实AI对话、VR、房间结构化数据、室内/室外路线、语音、员工SSO、素材上传后台和楼层点位编辑。不会用图上文字冒充可交互房间或可行走路径。
