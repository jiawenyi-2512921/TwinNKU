...
**2026-09-26 管理后台升级：**新增统一审核中心，集中查询点位、楼层与VR的待审内容；工作台真实统计、资料全局搜索、修改前后对照和操作记录搜索一并完善。部署需同时更新API与web，无新增迁移；[功能、验证与部署](docs/37-admin-workbench.md)。
...
第三批补齐楼层原图30秒超时、保留缩放位置的图片重试和旧事件隔离；前端68项回归与生产构建通过。原有资源验收脚本新增逐项JSON报告，区分HTTP读取、元数据和原图字节一致性；命令见[17严格验收](docs/17-floor-release.md)。不修改API、数据库、地图坐标或原图，不重导旧资料。

第四批补齐后台“楼层与 VR”建筑列表分页、搜索回到首页和失败重试，取消只显示前50项的限制；修复后台旧读取覆盖新结果及清空选择后仍转圈的问题。前端71项回归与构建通过，操作与验收见[18后台资源编辑](docs/18-resource-editor.md)和[36开发记录](docs/36-continuous-development.md)。
